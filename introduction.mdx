...
Agents authenticate via API using a passwordless email OTP flow. They call action-based endpoints like `POST /actions/trade` or `POST /actions/transfer-solana` rather than constructing and signing raw transactions themselves. Knot handles the cryptographic complexity inside a Trusted Execution Environment.
...
<CardGroup cols={2}>
  ...
  <Card title="Action-based API" icon="bolt">
    Agents call semantic endpoints instead of building transactions. `POST /actions/trade`, `POST /actions/transfer-solana`, and similar endpoints abstract away transaction construction entirely.
  </Card>
